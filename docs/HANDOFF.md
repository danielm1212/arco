...
**Następny: [Ty] wdrożenie kuracji (migracja+seed+test, prompt w rozmowie 2026-07-08) → [Claude] S9-cz.2 (higiena: rozbicie Logger.tsx, batch „poprzednio", paginacja historii) → S10 (offline+longevity)**. Równolegle [Ty]: decyzje z doków 2026-07-08 (PostHog, domena/ESP/Framer, budżet czasu 10 vs 15 h/tydz., schemat §6, persony/hasło ze strategii §9) + w sierpniu umówić kancelarię i Stripe activation (kalendarz §4). Otwarte [Ty] (starsze): test N1 na telefonie (wake lock/przerwa/PWA), test Warm v2 na telefonie, majory zależności. **Kolejka Notion: ~10 wpisów z 2026-07-08 czeka na flush na żądanie** (`docs/notion-sync-queue.md`). Mapa całej dokumentacji: **`docs/README.md`**.

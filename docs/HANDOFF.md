...
**Następny: S9-cz.2 (higiena: rozbicie Logger.tsx, batch „poprzednio", paginacja historii) → S10 (offline+longevity)**. Otwarte [Ty]: test N1 na telefonie (wake lock/przerwa/PWA — patrz `plan-sprintow-2026-07.md`), test Warm v2 na telefonie (logger jasny + neutralny canvas!), kuracja bazy, majory, 2 nowe programy. Na telefonie ~23 wpisy „Do testu [Ty]" w Notion; **kolejka `docs/notion-sync-queue.md` ma 3 pozycje jeszcze nie wypchnięte** (Warm v2 ikony/paleta, 2 programy) — czekają na „zsynchronizuj Notion" od Ciebie.

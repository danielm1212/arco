...
1. **Warstwa UX/UI:** gdzie ekipa mieszka w apce, jak wygląda ekran, kontekstowe zdarzenie na Home oraz stany puste/martwe (§4–5).
...
2. **Ekipa widzi:** „Daniel trenował dziś — 4. tydzień z rzędu” w swoim tabie; push tylko dla reakcji/nudge, NIE dla każdego check-inu — inaczej 4 osoby × 3 treningi = spam.
3. **Reakcja** 💪🔥👏🎯 jednym tapem → adresat widzi ją przy następnym wejściu do Ekipy + cichy push (batched, quiet hours).
4. **Cisza** → widoczna w hubie Ekipy (wygaszony płomień tygodnia) → **nudge** jednym tapem → „Radek przypomina Ci o treningu 💪" (push → tab Ekipy → e-mail fallback).
...
- **Digest tygodniowy (niedziela wieczór):** jedno zbiorcze podsumowanie ekipy — „Ania 3/3 🔥7 tyg. · Ty 2/3 · Radek 1/3". Kanał: hub Ekipy + (opt-in) e-mail. ✅ **W v1 (decyzja [Ty])** — najtańszy element podtrzymujący martwiejące ekipy; po odwołaniu concierge to także główna siatka bezpieczeństwa na scenariusz „ekipa żyje tylko dzięki podsumowaniom".
...
- **Martwa ekipa** (nikt ≥2 tyg.): NIE kasujemy, NIE wysyłamy „wasza ekipa umarła". Hub przechodzi w stan „Zbierz ich z powrotem" z jednym CTA-nudge do całości; Home może raz pokazać ten stan kontekstowo, ale nie utrzymuje stałej karty. Reaktywacja = czyjkolwiek check-in.
...
**✅ DECYZJA [Ty] 2026-07-16: Ekipa jest czwartym głównym tabem**, obok Treningu, Postępów i Historii. Zastępuje w bottom barze Ciało, które przechodzi pod Postępy.
- Home: brak stałej karty wejściowej. Może pojawić się najwyżej jedno kontekstowe, nowe zdarzenie z CTA do `/ekipa`; przy braku nowości moduł znika.
- Pełny ekran `/ekipa`: §5.2; przy >1 ekipie **segmented control / taby** u góry (switcher), ostatnio oglądana zapamiętana.
- Kropka nieprzeczytanego zdarzenia może pojawić się na tabie. Osobnego dzwonka/skrzynki nie budujemy, dopóki nie ma niezależnych zdarzeń poza Ekipą.
- Profil nie jest domem Ekipy: konto i preferencje są dostępne przez awatar, a społeczna pętla ma własną przestrzeń.
...
### 5.1 Kontekstowe zdarzenie na home
Nie jest stałą kartą nawigacyjną. Pojawia się tylko po nowym check-inie, reakcji lub istotnym stanie Ekipy, zawiera jedną linię i jedno CTA Otwórz, a po odczytaniu lub braku zdarzenia znika. Pełny status i wszystkie akcje mieszkają w tabie Ekipa.

### 5.2 Ekran `/ekipa`
- **Lista członków** (wiersz per osoba): awatar · imię · mini-FlameWeek bieżącego tygodnia (ten sam glif co badge celu — jeden glif ognia w całej apce) · badge „🔥 N tyg." · po prawej akcja kontekstowa: 💪 (reakcja na ostatni check-in) albo „szturchnij" (gdy warunki §3.2).

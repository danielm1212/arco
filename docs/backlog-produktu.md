...
| PLAN-05B | Komponent `ProgramCover` (zdjęcie / fallback) | Karta ma kotwicę wizualną nawet bez zdjęcia — gradient wg `focus_key` + istniejąca ikona 3D `plan`, zero nowych assetów | gotowe technicznie na `agent/plan-05b-program-cover`; czeka na PR/merge |

...
**L9 (znalezione przy F0.6): ✅ naprawione lokalnie 2026-07-20.**
Migracja `20260720153000_team_streak_warsaw.sql` liczy passę Ekipy z ukończonych
`sessions`, nie dziennych check-inów, wobec tego samego `weekly_goal`, którego F0.1
utrzymuje w zakresie aktywnego planu. `team_member_streak_weeks` zachowuje dokładnie
kontrakt `lib/week.ts`: niedokończony bieżący tydzień nie kasuje passy, ale 1 z 2
treningów nie tworzy jej. Wynik jest liczony dynamicznie w widoku Ekipy, więc korekta
daty nie zostawia starego snapshotu jako źródła prawdy. Zweryfikowane lokalnie migracją
i `smoke:team` na trzech jednorazowych kontach (1/2 → 0, 2/2 → 1).

**L10 (znalezione przy F0.6): ✅ naprawione lokalnie 2026-07-20.**
`get_pod_members`, obliczenie nudge'a i snapshot check-inu wyznaczają „dziś” przez
`now() at time zone 'Europe/Warsaw'`, więc granica tygodnia, liczba dni do końca i
antyspam nie zależą od domyślnej strefy sesji Postgresa. `weekly_done` używa teraz
ukończonych `sessions`, nie zdarzeń dziennych, oraz nie uwzględnia treningów sprzed
wspólnego członkostwa w Ekipie.
...
| F0.6 | Passa wg minimum planu (D4, wersja surowa) | **✅ Zrobione 2026-07-18; Ekipa wyrównana lokalnie 2026-07-20 przez L9/L10.** Nowa `weeksMeetingGoal(times, goal)` w `lib/week.ts` — tydzień liczy się do passy tylko przy `count >= goal` (nie ≥1). Dziś, Done, Postępy i Ekipa używają teraz tej samej semantyki celu; SQL Ekipy ma też jawną strefę Warszawy. Komunikat powrotu po przerwie bez winy: copy już istniejące w `WeeklyGoalBadge`, nie wymagało zmiany. | 0,5 d |
